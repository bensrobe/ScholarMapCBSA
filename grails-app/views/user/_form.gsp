...
<div class="panel-body" data-ng-controller="signupCtrl" id="editPanel">
    ...
    <div class="form-group">
        ...
        <div class="col-sm-6">
            <input type="text" class="form-control" name="url" placeholder="Not set"
                   value="${user.url}"/>
        </div>

        <div class="col-sm-3">
            <g:if test="${!lockConf.isUrlLocked}">
                <label class="switch switch-success">
                    <input type="checkbox" id="isUrlVisible"
                           class="permission" ${visConf?.isUrlVisible ? 'checked="unchecked"' : ''}/><i></i>
                </label>
            </g:if>
        </div>
    </div>
